def gammaintDL(x, psi0, E10, nounits = True, trimL = False, trimR = False,
               diagnostic = False):
    """Calculates gamma intrinsic by way of the Dalgarno-Lewis formalism. 
    ...
    Input:
        ...
        psi0 = array representing the ground state wave function. It is 
                ...
                or very nearly zero.
        E10 = first energy difference used to calculate gamma_max
    ...
    xbarF = simps((x-x00)*F*psi0**2, x)   # Will need this to compute G
    
    #Now repeat the above procedure for the inner integrand equal to (x-x00)*F-xbarF
    int2 = np.array([np.trapz( ((x[:i]-x00)*F[:i]-xbarF)*psi0[:i]**2, x[:i] ) 
                                for i in np.arange(1, len(x)+1)])
    
    #The next integral to determine the function G(x) is indefinite and is 
    #   allowed to have an undetermined constant so we take the lower bound to
    #   be the left side of the domain without loss of generality. We exclude 
    #   the endpoints anticipating a divergence in the inverse square psi0
    G = 2*m/hbar**2 * np.array(
        [np.trapz( psi0[1:i]**(-2)*int2[1:i], x[1:i])
                                for i in np.arange(2, len(x))]
                              )
    G = np.append(0,np.append(G,0))
    
    G = G - simps(G*psi0**2,x)    
    ...
    if diagnostic == True:
        ...
        plt.plot(x, x*F*G*psi0**2, label = '$xFG\psi_0^2$')
        plt.legend()
    
    gamma_max = 4*(e**4 * hbar**4 / m**2)/E10**5
    #Now calculate the total response with the last round of integrals
    gamma = -4*(simps(F*(x-x00)*G*psi0**2,x)-simps(x*F*psi0**2,x)*simps(F**2*psi0**2,x))
    gammaint = gamma / gamma_max
